...
export class Repository {
    ...
    deleteUserMap(id_user : number, id_map : number){
        ...
        return this.db.query(query, { id_user, id_map })
    }
    ...
    updateUserRol(id_user : number, id_layer : number, rol : string){
        ...
        return this.db.query(query, { id_layer, id_user });
    }

    deleteUserRol(id_user : number, id_layer : number){
        return this.db.query('DELETE FROM roles WHERE id_user = ${id_user} AND id_layer = ${id_layer}', { id_user, id_layer })
    }
    ...
    deleteUserGroup(id_user : number, group : string){
        return this.db.query('DELETE FROM user_groups WHERE "id_user" = ${id_user} AND "group" = ${group}', { id_user, group });
    }
    ...
    deleteGroup(id : number){
        return this.db.query('DELETE FROM groups WHERE id = ${id}', { id });
    }

    createMap(name : string){
        let cs = new this.pgp.helpers.ColumnSet(['name'], {table: 'maps'});
        let values = [{ name }];
        let query = this.pgp.helpers.insert(values, cs) + ' RETURNING *';
        return this.db.query(query);
    }

    deleteMap(id_map : number){
        return this.db.query('DELETE FROM maps WHERE id = ${id_map}', { id_map });
    }
    ...
    deleteDefaultMap(id_map : number){
        return this.db.query('DELETE FROM default_maps WHERE id = ${id_map}', { id_map });
    }
    ...
    deleteMapLayer(id_map : number, id_layer : number){
        return this.db.query('DELETE FROM map_layers WHERE id_map = ${id_map} AND id_layer = ${id_layer}', { id_map, id_layer });
    }
    ...
    deleteMapBaselayer(id_map : number, id_base_layer : number){
        return this.db.query('DELETE FROM map_base_layers WHERE id_map = ${id_map} AND id_base_layer = ${id_base_layer}', { id_map, id_base_layer });
    }
    ...
    createBaselayer(service_url : string, layers : string){
        return this.db.one("INSERT INTO base_layers(service_url, name) VALUES('${service_url#}', '${layers#}') RETURNING *", { service_url, layers });
    }
